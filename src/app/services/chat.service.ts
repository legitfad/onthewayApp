...
export class ChatService {
  ...
  getChatGroups() {
    return this.db.collection(`users/${this.auth.currentUserId}/groups`).snapshotChanges().pipe(
      map(actions => actions.map(a => {
        ...
        return this.getOneChatGroup(data['id'], user_group_key);
      }))
    )
  }

  getOneChatGroup(id, user_group_key = null) {
  ...
  leaveChatGroup(groupId, users) {
    return this.getChatGroups().pipe(
